...
SSH to `robot@<mac-mini>` and install Docker Desktop (via Homebrew cask) plus enable it. The Mini has an M4 (Apple Silicon) and no Docker yet.

```bash
ssh robot@<mac-mini-ip> \
  '/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"' && \
ssh robot@<mac-mini-ip> 'brew install --cask docker' 
```
...
ssh robot@<mac-mini-ip> 'git clone https://github.com/Yihong89/deepcards.git ~/deepcards && cd ~/deepcards'
```

- [ ] **Step 3: Build + run**

```bash
ssh robot@<mac-mini-ip> 'cd ~/deepcards/deploy && DEEPCHARD_SECRET=$(openssl rand -hex 32) DEEPCHARD_ADMIN_USER=admin DEEPCHARD_ADMIN_PASS=<choose> docker compose up -d --build'
```
